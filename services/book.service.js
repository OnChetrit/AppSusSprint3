...
const gSearch = storageService.loadFromStorage('searchDB') || [];
...
let gBooks = [
  {
    ...
    reviews: [
      { txt: 'Hello', name: 'Yoav', rate: 4, date: '18.4.2021' },
      { txt: 'Nice', name: 'Yosi', rate: 2, date: '18.3.2021' },
    ],
  },
  {
    id: 'JYOJa2NpSCq',
...
function _createReview(name, txt, rate) {
  return {
...
function removeReview(reviewId, reviews) {
...
function addReview(review, bookId) {
  const { name, txt, rate } = review;
  const newReview = _createReview(name, txt, rate);
...
function getRate(book) {
  let finalRate = 0;
  if (!book) return;
  for (let i = 0; i < book.reviews.length; i++) {
    finalRate += book.reviews[i].rate;
  }
  return Promise.resolve({
    rate: finalRate / book.reviews.length,
...
function getBooksFromGoogle(title) {
  return new Promise((resolve, reject) => {
    ...
    axios
        ...
        if (booksList) {
          booksList.map((book) => {
            const myTypeBook = {
              ...
              categories: book.volumeInfo.categories || ['Computers', 'Hack'],
              thumbnail:
      ...
      .catch((err) => {
        reject('Cannot reach server:', err);
      });
  });
}
