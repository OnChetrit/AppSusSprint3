    ...
    return (
      <div className="mail-sort">
        <div className="sort-container">
          <button onClick={ () => this.onSetSelected()}
            title="Select All"
            className={`btn selecting`}>
              {selected ? <img src="../img/mail/unselect.png" /> : <img src="../img/mail/select.png" />}
              {/* <img src="../img/mail/select.png" /> */}
            </button>
            <button onClick={() => {
              this.onSetSort('title');
            }}
            className={`sort sort-title`}><span>Title</span>
                
            </button>
            <button onClick={() => {
              this.onSetSort('subject');
            }}
            className={`sort sort-subject`}><span>Subject</span>
            </button>
            <button onClick={() => {
              this.onSetSort('date');
            }}
            className={`sort sort-date`}><span>Date</span>
            </button>
            <button onClick={ () => this.onSetRemoveSelected()}
            className={`selected remove-selected`}>Remove Selected
            </button>
            <button onClick={ () => this.onSetRemoveSelectedFromTrash()}
            className={`selected remove-selected`}>Remove trash Selected 
            </button>
            <button onClick={ () => this.onSetRestoreSelected()}
            className={`selected restore-selected`}>Restore Selected
            </button>
            <button onClick={ () => this.onSetSelectedToArchive()}
            className={`selected archive-selected`}>Selected to archive
            </button>
            <button onClick={ () => this.onSetSelectedRead()}
            className={`selected read-selected`}>Selected to read/unread
            </button>
        </div>
      </div>
    );
